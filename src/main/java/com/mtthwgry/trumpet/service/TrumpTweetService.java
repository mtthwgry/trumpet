package com.mtthwgry.trumpet.service;
...
// Shamelessly ripped from: https://rosettacode.org/wiki/Markov_chain_text_generator#Java
public class TrumpTweetService {

  private static Random r = new Random();

  public static String markov(int keySize, int outputSize) throws IOException, URISyntaxException {
    if (keySize < 1) throw new IllegalArgumentException("Key size can't be less than 1");
    Path path = Paths.get(TrumpTweetService.class.getResource("/tweets.txt").toURI());
    byte[] bytes = Files.readAllBytes(path);
    String[] words = Arrays.stream(new String(bytes).trim().split(" "))
        .filter(word -> !word.matches(".*http.+"))
    ...
    String[] prefixes = dict.keySet().stream()
        .filter(word -> word.charAt(0) == word.toUpperCase().charAt(0))
        .toArray(String[]::new);

    int rn = r.nextInt(prefixes.length);
    String prefix = prefixes[rn];
